    ...
    public class RPModule : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (checkRects)
            {
                ...
                // Tentativa usando um foreach.
                if (rectsRegularTrainer.Count > 0)
                {
                    int i = 0;
                    while (i < rectsRegularTrainer.Count)
                    {
                        for (int j = 0; j < rectsRegularTrainer.Count; j++)
                        {
                            if (rectsRegularTrainer[i].Contains(rectsRegularTrainer[j]))
                            {
                ...
                /*
                // Tentativa utilizando dois rects auxiliares
                Rect rectA = rectsRegularTrainer[0];
                Rect rectB = rectsRegularTrainer[1];

                while (rectsRegularTrainer[rectsRegularTrainer.IndexOf(rectA) + 1] != null)
                {
                    ...
                    rectA = rectB;
                    rectB = rectsRegularTrainer[rectsRegularTrainer.IndexOf(rectA) + 1];
                }
                */
            ...
            if (endRecognizing)
            {
                ...
                if (percentageOfHitsRegular >= minDetectedsForRegular || countVerifierForMirror >= minDetectedsForMirror)
                {
