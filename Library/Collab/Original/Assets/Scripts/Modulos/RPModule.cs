    ...
    public class RPModule : MonoBehaviour
    {
        ...
        void Update()
        {
                ...
                while (rectsRegularTrainer.Count < 50)
                {
                    foreach (Texture2D image in screenshots)
                    {
                        ...
                        var detectedOnRegular = regularCascade.DetectMultiScale(
                            ...
                            scaleFactor: 1.05,
                            minNeighbors: 3,
                            flags: HaarDetectionType.FindBiggestObject | HaarDetectionType.DoRoughSearch | HaarDetectionType.ScaleImage,
            ...
            if (checkRects)
            {
                ...
                // Tentativa usando dois fors.
                if (rectsRegularTrainer.Count > 0)
                {
                    for (int i = 0; i < rectsRegularTrainer.Count; i++)
                    {
                        for (int j = 0; j < rectsRegularTrainer.Count; j++)
                        {
                            if (rectsRegularTrainer[i].IntersectsWith(rectsRegularTrainer[j]))
                            {
            ...
            if (endRecognizing)
            {
                ...
                {
                    StartCoroutine(EndOfRecognizing(true)); // Recognized.
                }
                else
                {
                    StartCoroutine(EndOfRecognizing(false)); // Not recognized.
                }
            }
        }
